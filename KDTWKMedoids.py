...
Implementation according (more or less) to AEON toolbox API, specifically:
https://github.com/aeon-toolkit/aeon/blob/main/aeon/clustering/_k_medoids.py
...
def pairwise_distance(x: np.ndarray, y: np.ndarray, method: Callable, sigma: float, epsilon: float):
    out = np.zeros((len(x),len(y)))
    for i in range(len(x)):
        for j in range(len(y)):
            out[i,j] = -np.log(TEKA.kdtw(x[i],y[j], sigma, epsilon)+1e-300)
    return out
    
def kdtw_distance(x: np.ndarray, y: np.ndarray, sigma: float, epsilon: float):
    return -np.log(TEKA.kdtw(x,y,sigma, epsilon)+1e-300)
...
class KDTWKMedoids():
    ...
    def __init__(
        ...
        distance: Union[str, Callable] = kdtw_distance,
        distance_params: dict = {"sigma": 1.0, "epsilon":1e-300},
        method: str = "pam",
        n_init: int = 10,
        max_iter: int = 300,
        tol: float = 1e-6,
        ...
        self._distance_callable = kdtw_distance
